...
# Usage: python bag2csv.py robot_name joint_id bagfile csvfile
#
...
def main():
    ...
    joint_id = int(sys.argv[2])                     # The Dynamixel ID of the joint to be analyzed
    ...
    for topic, msg, t in bag.read_messages(topics=['/'+robot_name+'/joint_temperatures']):
            temp_start_time = t.to_sec()
            break
    for topic, msg, t in bag.read_messages(topics=['/'+robot_name+'/joint_states']):
            states_start_time = t.to_sec()
            break
    for topic, msg, t in bag.read_messages(topics=['/'+robot_name+'/joint/commands']):
    ...
    for topic, msg, t in bag.read_messages(topics=['/'+robot_name+'/joint_temperatures']):
        ...
        data.append(msg.temps[joint_id])
        temp_list.append(data)
        min_temp = min(min_temp, msg.temps[joint_id])
        max_temp = max(max_temp, msg.temps[joint_id])
    ...
    for topic, msg, t in bag.read_messages(topics=['/'+robot_name+'/joint_states']):
        if (cntr % modulo_num == 0):
            ...
            data.append(msg.position[joint_id])
            data.append(msg.velocity[joint_id])
            data.append(msg.effort[joint_id])
            state_list.append(data)
        if msg.effort[joint_id] > 0:
            sum_pos_effort += msg.effort[joint_id]
            num_pos_effort += 1
        elif msg.effort[joint_id] < 0:
            sum_neg_effort += msg.effort[joint_id]
            num_neg_effort += 1
        min_effort = min(min_effort, msg.effort[joint_id])
        max_effort = max(max_effort, msg.effort[joint_id])
        cntr += 1

    # Since there are many joint/commands messages, only one out of every nine messages will
    # be inputted into the csv file ('nine' was arbitrarily chosen)
    cntr = 0
    for topic, msg, t in bag.read_messages(topics=['/'+robot_name+'/joint/commands']):
        if (cntr % modulo_num == 0):
    ...
    min_size = min(len(state_list), len(command_list))
